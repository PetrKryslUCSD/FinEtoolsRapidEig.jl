...
apptimpl(neigvss, ts) = begin
	@pgf Plot({
		mark = "x", mark_size = 5, dotted,
		color = "black", very_thick
		},
		Table([neigvss, ts])
		)
...
for jsn = [
	"FullApproxlug-lug-217666_nas-neigvs=15-tol=0_01.json",                                                             
	"FullApproxlug-lug-217666_nas-neigvs=50-tol=0_01.json",                           
	"FullApproxlug-lug-217666_nas-neigvs=150-tol=0_01.json",                                                            
	"FullApproxlug-lug-217666_nas-neigvs=300-tol=0_01.json",                                    
	"FullApproxlug-lug-217666_nas-neigvs=600-tol=0_01.json",                           
	]
	updatets!(trs2, timingtotal, jsn)
end
figurename = getcleanname("Redlug-FullApproxlug-lug-217666_nas-neigvs=$(neigvss)-LegendreBasis-a=1_5-tol=0_02") * ".pdf"
@show trs1, tfs, trs2
@. trs1 = tfs / trs1
@. trs2 = tfs / trs2
push!(plots, redtimpl1(neigvss, trs1), apptimpl(neigvss, trs2))
...
list = [n for n in readdir(".") if match(r"Ful.*217666_nas.*.json", n) != nothing];
for item in list
 	println("\"$item\",")
 end 
